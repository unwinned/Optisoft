...
class MainRunner(ABC):
    ...
    async def prepare_run(self):
        ...
        tasks = [asyncio.create_task(self.run_task_with_retry(client, proxy, self.action, barrier)) 
                 for client, proxy in zip(clients, proxies)]
...
class ModernRunner:
    ...
    async def run_task_with_retry(self, data):
        client = data['client']
        proxy = data['proxy']
        proxy_str = proxy.session_proxy.get('http') if proxy.session_proxy else None
        logger = Logger(client.address, additional={'pk': client.key, 'proxy': proxy_str}).logger
